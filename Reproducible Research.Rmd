...
Given below are the steps taken to conduct the data analysis.
...
```{r t-series, avg steps, echo=TRUE}
time.series <- df %>% 
        group_by(date) %>% 
...
Using the *time.series* dataframe, here we create the time series plot for the avearge number of steps take per days. Note that to run this plot, you will need to make *lubradiate* package available which is used for date transformation.
...
```{r ppm imputing, echo=FALSE, cache=TRUE}
...
```{r select and inspect, echo=TRUE}
